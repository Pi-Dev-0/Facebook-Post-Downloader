...
import { resolveInstagramPlaceholder } from "../instagram/download.js";
...
async function* fetchAttachments(story) {
  ...
  if (isInsta) {
    ...
    // Instagram placeholder: delegate entirely to the Instagram module
    if (s.placeholder && (s.shortcode || s.id)) {
      try {
        yield* resolveInstagramPlaceholder(s, globalStoriesCache);
        return;
      } catch (err) {
        console.warn("[fpdl] Instagram resolution failed", err);
      }
    }
